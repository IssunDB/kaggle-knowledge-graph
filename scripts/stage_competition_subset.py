...
DEFAULT_STAGE_DIR = Path(os.environ.get("STAGE_DIR", "issundb/stage_competition"))

# Competitions enabled on or after this instant are in scope.
ENABLED_SINCE = "2020-01-01"
# Top kernels per competition by votes form the curated code layer.
...
def create_seed_tables(
    ...
    # Seed kernels: top N kernels per competition by votes (the curated code layer).
    con.execute(
        f"""
    ...
    # Seed users: everyone implicated by the teams, submissions, kernels, or discussion.
    con.execute(
        """
...
def stage_nodes(
    ...
    copy_csv(
        ...
        """,
        stage_dir / "nodes_submission.csv",
    )
    copy_csv(
        con,
        """
...
def stage_edges(con: duckdb.DuckDBPyConnection, meta_dir: Path, stage_dir: Path) -> None:
    ...
    copy_csv(
        ...
        """,
        stage_dir / "edges_submission_from_kernel_version.csv",
    )
    ...
    copy_csv(
        ...
        """,
        stage_dir / "edges_team_public_leaderboard_submission.csv",
    )
    copy_csv(
        con,
        """
        ...
        """,
        stage_dir / "edges_team_private_leaderboard_submission.csv",
    )
    # Code layer.
    copy_csv(
        con,
