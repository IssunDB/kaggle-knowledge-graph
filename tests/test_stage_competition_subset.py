...
class TestDanglingEndpoints:
    ...
    def test_user_edges_skip_users_missing_from_users_csv(
        ...
        append_csv_row(meta_dir, "TeamMemberships.csv", {"Id": 2, "TeamId": 101, "UserId": 555})
        append_csv_row(
            meta_dir,
            "ForumMessages.csv",
            {
        ...
        for name in (
            "edges_user_member_of_team.parquet",
            "edges_user_led_team.parquet",
            "edges_user_submitted.parquet",
        ...
        # The message itself stays staged; only its author edge is dropped.
        messages = pl.read_parquet(stage_dir / "nodes_forum_message.parquet")
    ...
    def test_organization_edges_skip_organizations_missing_from_csv(
        ...
        append_csv_row(
            meta_dir,
            "KernelVersionDatasetSources.csv",
            {"Id": 9, "KernelVersionId": 11, "SourceDatasetVersionId": 503},
        )
        stage_dir = tmp_path / "stage"
        _run_pipeline(meta_dir, stage_dir)
        organizations = set(pl.read_parquet(stage_dir / "nodes_organization.parquet")["Id"])
        assert 777 not in organizations
        assert 402 in set(pl.read_parquet(stage_dir / "nodes_dataset.parquet")["Id"])
        for name in (
